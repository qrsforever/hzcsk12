...
_RPCClient = None
_RPCEnable = -1
K12CV_TOKEN, K12CV_OP, K12CV_USER, K12CV_UUID = None, None, None, None


def hzcsk12_send_message(msgtype, message, end=False):
    global _RPCClient, _RPCEnable, K12CV_TOKEN, K12CV_OP, K12CV_USER, K12CV_UUID
    ...
    if _RPCEnable == -1:
        ...
        if not host or not port:
            _RPCEnable = 0
            return
        K12CV_TOKEN = os.environ.get('K12CV_TOKEN', 'Unkown')
    ...
    try:
        if message:
            _RPCClient.send_message(K12CV_TOKEN, K12CV_OP, K12CV_USER, K12CV_UUID, msgtype, message)
        if end:
            _RPCClient.close()
    except Exception:
        pass
