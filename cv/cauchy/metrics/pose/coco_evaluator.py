...
class CocoEvaluator(object):
    ...
    def relabel(self, json_dir, method="rpose"):
        submission_file = os.path.join(
            json_dir, "person_keypoints_val2017_{}_results.json".format(method)
        )
        img_id_list = list()
        object_list = list()
            ...
            with open(json_path, "r") as json_stream:
                ...
                for object in info_tree["objects"]:
                    ...
                    for j in range(self.configer.get("data", "num_kpts") - 1):
                        keypoint = object["kpts"][
                            self.configer.get("details", "coco_to_ours")[j]
                        ]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        dest="gt_file",
        help="The groundtruth annotations file of coco keypoints.",
    )
    parser.add_argument(
        "--json_dir",
        default=None,
