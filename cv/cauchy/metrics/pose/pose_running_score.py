...
class PoseRunningScore(object):
    ...
    def compute_oks(self, gt_kpts, pred_kpts):
        ...
        for i in range(gt_count):
            ...
            scale = (
                max(
                    np.max(anno_keypoints[:, 0]) - np.min(anno_keypoints[:, 0]),
                    np.max(anno_keypoints[:, 0]) - np.min(anno_keypoints[:, 0]),
                )
                ** 2
                + 1e-8
            ...
            if np.sum(visible) == 0:
                ...
                for j in range(pred_count):
                    predict_keypoints = np.reshape(
                        np.array(pred_kpts[j]),
                        (self.configer.get("data", "num_keypoints"), 3),
                    )
                    dis = np.sum(
                        (
                            anno_keypoints[visible, :2]
                            - predict_keypoints[visible, :2]
                        )
                        ** 2,
                        axis=1,
                    )
                    oks[i, j] = np.mean(
                        np.exp(
                            -dis
                            / 2
                            / self.configer.get("details", "delta")[visible]
                            ** 2
                            / (scale + 1)
                        )
                    )
    ...
    def update(self, batch_pred_kpts, batch_gt_kpts):
        ...
        for i in range(len(batch_pred_kpts)):
            ...
            self.oks_all = np.concatenate(
                (self.oks_all, np.max(oks, axis=1)), axis=0
            )
    ...
    def get_mAP(self):
        ...
        for threshold in np.linspace(0.5, 0.95, 10):
            average_precision.append(
                np.sum(self.oks_all > threshold) / np.float32(self.oks_num)
            )
