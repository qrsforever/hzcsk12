    ...
    def __init__(self, data_path=None, out_path=None, predict_path=None):
        ...
        if "CITYSCAPES_EXPORT_DIR" in os.environ:
            ...
            if not os.path.isdir(export_dir):
                raise ValueError(
                    "CITYSCAPES_EXPORT_DIR {} is not a directory".format(
                        export_dir
                    )
                )
            self.exportFile = "{}/resultPixelLevelSemanticLabeling.json".format(
                export_dir
            )
        else:
            self.exportFile = os.path.join(
                out_path,
                "../evaluationResults",
...
def getPrediction(args, groundTruthFile):
    ...
    filePattern = "{}_{}_{}*.png".format(
        csFile.city, csFile.sequenceNb, csFile.frameNb
    )
...
def getIouScoreForCategory(category, confMatrix, args):
    ...
    fp = np.longlong(
        confMatrix[notIgnoredAndNotInCategory, :][:, labelIds].sum()
    )
...
def printConfMatrix(confMatrix, args):
    # print line
    print(
        "\b{text:{fill}>{width}}".format(width=15, fill="-", text=" "), end=" "
    )
    for label in args.evalLabels:
        print(
            "\b{text:{fill}>{width}}".format(
                width=args.printRow + 2, fill="-", text=" "
            ),
            end=" ",
        )
    print(
        "\b{text:{fill}>{width}}".format(
            width=args.printRow + 3, fill="-", text=" "
        )
    )
    ...
    for label in args.evalLabels:
        print(
            "\b{text:^{width}} |".format(
                width=args.printRow, text=id2label[label].name[0]
            ),
            end=" ",
        )
        ...
        for y in range(0, len(confMatrix[x])):
            ...
            print(
                ...
                + "\b{text:>{width}.2f}  ".format(
                    width=args.printRow, text=matrixFieldValue
                )
                + args.nocol,
                end=" ",
            )
...
class EvalPixel:
    def __init__(self, args, predictionImgList=None, groundTruthImgList=None):
        ...
        if predictionImgList is None or groundTruthImgList is None:
            self.groundTruthImgList, self.predictionImgList = self.getDefaultData(
                self.args
            )
    ...
    def evaluateImgLists(self, predictionImgList, groundTruthImgList, args):
        ...
        if not args.quiet:
            print(
                "Evaluating {} pairs of images...".format(
        ...
        if not args.quiet:
            ...
            niouAvgStr = (
                ...
                + "{avg:5.3f}".format(
                    avg=getScoreAverage(classInstScoreList, args)
                )
                + args.nocol
            )
            print("--------------------------------")
        ...
        if not args.quiet:
            ...
            iouAvgStr = (
                ...
                + "{avg:5.3f}".format(
                    avg=getScoreAverage(categoryScoreList, args)
                )
                + args.nocol
            )
            niouAvgStr = (
                getColorEntry(
                    getScoreAverage(categoryInstScoreList, args), args
                )
                + "{avg:5.3f}".format(
                    avg=getScoreAverage(categoryInstScoreList, args)
                )
                + args.nocol
            )
            print("--------------------------------")
    ...
    def evaluatePair(
        ...
        if CSUPPORT:
            ...
            for (groundTruthImgPixel, predictionImgPixel) in izip(
                groundTruthImg.getdata(), predictionImg.getdata()
            ):
        ...
        if args.evalInstLevelScore:
            ...
            for category in instanceStats["categories"]:
                categoryMasks[category] = np.in1d(
                    predictionNp,
                    instanceStats["categories"][category]["labelIds"],
        ...
        if args.evalPixelAccuracy:
            ...
            notIgnoredPixels = np.in1d(
                groundTruthNp, notIgnoredLabels, invert=True
            ).reshape(groundTruthNp.shape)
            erroneousPixels = np.logical_and(
                notIgnoredPixels, (predictionNp != groundTruthNp)
            )
            perImageStats[predictionImgFileName] = {}
            perImageStats[predictionImgFileName][
                "nbNotIgnoredPixels"
            ] = np.count_nonzero(notIgnoredPixels)
            perImageStats[predictionImgFileName][
                "nbCorrectPixels"
            ] = np.count_nonzero(erroneousPixels)

        return nbPixels

    # launch the process
    def run(self):
        self.evaluateImgLists(
            self.predictionImgList, self.groundTruthImgList, self.args
        )
...
class CityscapesEvaluator(object):
    def evaluate(self, pred_dir=None, gt_dir=None):
        ...
        args = CArgs(
            data_path=data_path, out_path=pred_path, predict_path=pred_path
        )
        ob = EvalPixel(args)
        ob.run()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        dest="gt_dir",
        help="The directory of ground truth.",
    )
    parser.add_argument(
        "--pred_dir",
        default=None,
        type=str,
        dest="pred_dir",
        help="The directory of predicted labels.",
    )
