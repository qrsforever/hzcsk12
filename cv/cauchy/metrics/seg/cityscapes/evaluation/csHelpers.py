...
try:
    ...
    from metric.seg.cityscapes.helpers.labels import (
        labels,
        name2label,
        id2label,
        trainId2label,
...
def getCsFileInfo(fileName):
    ...
    if len(parts) == 5:
        ...
        printError(
            "Found {} part(s) in given filename ({}). Expected 5 or 6.".format(
                len(parts), fileName
            )
        )
...
def writeDict2JSON(dictName, fileName):
    with open(fileName, "w") as f:
        f.write(
            json.dumps(
                dictName, default=lambda o: o.__dict__, sort_keys=True, indent=4
            )
        )
