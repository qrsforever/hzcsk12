...
class CocoEvaluator(object):
    ...
    def relabel(self, json_dir, method="mask_rcnn"):
        submission_file = os.path.join(
            json_dir, "person_instances_val2017_{}_results.json".format(method)
        )
        img_id_list = list()
        object_list = list()
            ...
            with open(json_path, "r") as json_stream:
                ...
                for object in info_tree["objects"]:
                    ...
                    object_dict["category_id"] = int(
                        self.configer.get("data", "coco_cat_seq")[
                            object["label"]
                        ]
                    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        dest="gt_file",
        help="The groundtruth annotations file of coco instances.",
    )
    parser.add_argument(
        "--json_dir",
        default=None,
