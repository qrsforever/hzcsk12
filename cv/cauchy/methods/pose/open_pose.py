...
class OpenPose(object):
    ...
    def train(self):
        ...
        self.scheduler.step(
            self.train_schedule_loss.avg, epoch=self.configer.get("epoch")
        )
        ...
        for i, data_dict in enumerate(self.train_loader):
            ...
            loss_heatmap = self.mse_loss(
                heatmap_out, heatmap, mask=maskmap, weights=self.weights
            )
            loss_associate = self.mse_loss(
                paf_out, vecmap, mask=maskmap, weights=self.weights
            )
            ...
            self.train_loss_associate.update(
                loss_associate.item(), inputs.size(0)
            )
    ...
    def val(self):
        ...
        with torch.no_grad():
            for i, data_dict in enumerate(self.val_loader):
                ...
                self.val_loss_heatmap.update(
                    loss_heatmap.item(), inputs.size(0)
                )
                self.val_loss_associate.update(
                    loss_associate.item(), inputs.size(0)
                )
            ...
            RunnerHelper.save_net(
                self, self.pose_net, val_loss=self.val_losses.avg
            )
            Log.info(
                "Loss Heatmap:{}, Loss Asso: {}".format(
