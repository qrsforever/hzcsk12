...
from cauchy.utils.parser.pose_parser import PoseParser
from cauchy.utils.visualizer.pose_visualizer import PoseVisualizer
...
class OpenPoseTest(object):
    ...
    def __test_img(self, image_path, json_path, raw_path, vis_path):
        ...
        heatmap_avg = np.zeros(
            (ori_height, ori_width, self.configer.get("network", "heatmap_out"))
        )
        paf_avg = np.zeros(
        ...
        for i, scale in enumerate(multiplier):
            ...
            with torch.no_grad():
                ...
                heatmap = (
                    heatmap_out.squeeze(0).cpu().numpy().transpose(1, 2, 0)
                )
                ...
                heatmap = cv2.resize(
                    heatmap[: border_hw[0], : border_hw[1]],
    ...
    def __get_info_tree(self, image_raw, subset, candidate):
        ...
        for n in range(len(subset)):
            ...
            object_dict["kpts"] = np.zeros(
                (self.configer.get("data", "num_kpts"), 3)
            ).tolist()
    ...
    def __extract_paf_info(self, img_raw, paf_avg, all_peaks):
        ...
        for k in range(len(self.configer.get("details", "limb_seq"))):
            ...
            candA = all_peaks[
                self.configer.get("details", "limb_seq")[k][0] - 1
            ]
            candB = all_peaks[
                self.configer.get("details", "limb_seq")[k][1] - 1
            ]
            nA = len(candA)
            nB = len(candB)
            if nA != 0 and nB != 0:
                ...
                for i in range(nA):
                    for j in range(nB):
                        ...
                        norm = (
                            math.sqrt(vec[0] * vec[0] + vec[1] * vec[1]) + 1e-9
                        )
                        ...
                        num_positive = len(
                            np.nonzero(
                                score_midpts
                                > self.configer.get("res", "limb_threshold")
                            )[0]
                        )
    ...
    def __get_subsets(self, connection_all, special_k, all_peaks):
        ...
        for k in self.configer.get("details", "mini_tree"):
            if k not in special_k:
                ...
                indexA, indexB = (
                    np.array(self.configer.get("details", "limb_seq")[k]) - 1
                )
    ...
    def debug(self, vis_dir):
        for i, data_dict in enumerate(self.pose_data_loader.get_trainloader()):
            ...
            for j in range(inputs.size(0)):
                ...
                mask_canvas = (
                    maskmap[j].repeat(3, 1, 1).numpy().transpose(1, 2, 0)
                )
