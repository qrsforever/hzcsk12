...
from cauchy.datasets.seg.data_loader import DataLoader
from cauchy.methods.tools.blob_helper import BlobHelper
from cauchy.methods.tools.runner_helper import RunnerHelper
from cauchy.models.seg.model_manager import ModelManager
from cauchy.utils.helpers.image_helper import ImageHelper
from cauchy.utils.tools.logger import Logger as Log
from cauchy.utils.parser.seg_parser import SegParser
from cauchy.utils.visualizer.seg_visualizer import SegVisualizer
...
class FCNSegmentorTest(object):
    ...
    def _get_blob(self, ori_image, scale=None):
        ...
        if self.configer.exists("test", "input_size"):
            ...
            "test", "min_side_length"
        ) and not self.configer.exists("test", "max_side_length"):
            image = self.blob_helper.make_input(
                image=ori_image,
            ...
            "test", "min_side_length"
        ) and self.configer.exists("test", "max_side_length"):
            image = self.blob_helper.make_input(
                image=ori_image,
    ...
    def test_img(self, image_path, label_path, vis_path, raw_path):
        ...
        if self.configer.get("test", "mode") == "ss_test":
            ...
            Log.error(
                "Invalid test mode:{}".format(self.configer.get("test", "mode"))
            )
            exit(1)
        ...
        ori_img_bgr = ImageHelper.get_cv2_bgr(
            ori_image, mode=self.configer.get("data", "input_mode")
        )
    ...
    def ss_test(self, ori_image):
        ...
        results = cv2.resize(
            results[: border_hw[0], : border_hw[1]],
    ...
    def sscrop_test(self, ori_image):
        ...
        results = cv2.resize(
            results, (ori_width, ori_height), interpolation=cv2.INTER_CUBIC
        )
        total_logits += results
        return total_logits
    ...
    def mscrop_test(self, ori_image):
        ...
        for scale in self.configer.get("test", "scale_search"):
            ...
            crop_size = self.configer.get("test", "crop_size")
            if (
                image.size()[3] > crop_size[0]
                and image.size()[2] > crop_size[1]
            ):
                results = self._crop_predict(image, crop_size)
            else:
                results = self._predict(image)
    ...
    def _crop_predict(self, image, crop_size):
        ...
        for height in height_starts:
            for width in width_starts:
                image_crop = np_image[
                    height : height + crop_size[1], width : width + crop_size[0]
                ]
                split_crops.append(image_crop[np.newaxis, :])

        split_crops = np.concatenate(
            split_crops, axis=0
        )  # (n, crop_image_size, crop_image_size, 3)
        inputs = (
            torch.from_numpy(split_crops).permute(0, 3, 1, 2).to(self.device)
        )
        with torch.no_grad():
    ...
    def _decide_intersection(self, total_length, crop_length):
        stride = int(
            crop_length * self.configer.get("test", "crop_stride_ratio")
        )  # set the stride as the paper do
    ...
    def __relabel(self, label_map):
        ...
        for i in range(self.configer.get("data", "num_classes")):
            label_dst[label_map == i] = self.configer.get("data", "label_list")[
                i
            ]
    ...
    def debug(self, vis_dir):
        ...
        for i, data_dict in enumerate(self.seg_data_loader.get_trainloader()):
            ...
            for j in range(inputs.size(0)):
                ...
                image_canvas = self.seg_parser.colorize(
                    label_map, image_canvas=image_bgr
                )
                cv2.imwrite(
