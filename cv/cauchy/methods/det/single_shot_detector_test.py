...
from cauchy.models.det.layers.ssd_priorbox_layer import SSDPriorBoxLayer
from cauchy.models.det.layers.ssd_target_generator import SSDTargetGenerator
...
class SingleShotDetectorTest(object):
    ...
    def __test_img(self, image_path, json_path, raw_path, vis_path):
        ...
        ori_img_bgr = ImageHelper.get_cv2_bgr(
            img, mode=self.configer.get("data", "input_mode")
        )
        ...
        batch_detections = self.decode(
            ...
            self.ssd_priorbox_layer(
                feat_list, self.configer.get("test", "input_size")
            ),
            self.configer,
            [inputs.size(3), inputs.size(2)],
        )
        json_dict = self.__get_info_tree(
            batch_detections[0], ori_img_bgr, [inputs.size(3), inputs.size(2)]
        )
    ...
    def decode(bbox, conf, default_boxes, configer, input_size):
        ...
        labels = (
            labels.view(1, 1, -1, 1)
            .repeat(batch_size, num_priors, 1, 1)
            .contiguous()
            .view(batch_size, -1, 1)
        )
    ...
    def debug(self, vis_dir):
        ...
        for i, data_dict in enumerate(self.det_data_loader.get_trainloader()):
            ...
            for j in range(inputs.size(0)):
                ...
                json_dict = self.__get_info_tree(
                    batch_detections[j], ori_img_bgr, input_size
                )
                image_canvas = self.det_parser.draw_bboxes(
                    ori_img_bgr.copy(),
                    json_dict,
