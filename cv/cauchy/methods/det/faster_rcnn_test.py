...
from cauchy.utils.helpers.det_helper import DetHelper
from cauchy.utils.helpers.image_helper import ImageHelper
from cauchy.utils.helpers.json_helper import JsonHelper
from cauchy.models.det.layers.fr_priorbox_layer import FRPriorBoxLayer
from cauchy.models.det.layers.fr_roi_generator import FRROIGenerator
from cauchy.models.det.layers.fr_roi_sampler import FRROISampler
from cauchy.models.det.layers.rpn_target_assigner import RPNTargetAssigner
from cauchy.utils.tools.logger import Logger as Log
from cauchy.utils.parser.det_parser import DetParser
...
from cauchy.utils.helpers.dc_helper import DCHelper
...
class FastRCNNTest(object):
    ...
    def __test_img(self, image_path, json_path, raw_path, vis_path):
        ...
        ori_img_bgr = ImageHelper.get_cv2_bgr(
            image, mode=self.configer.get("data", "input_mode")
        )
        ...
        if self.configer.exists("test", "fit_stride"):
            ...
            expand_image = torch.zeros((b, c, h + pad_h, w + pad_w)).to(
                inputs.device
            )
        ...
        data_dict = dict(
            ...
            meta=DataContainer(
                [
                    [
                        dict(
                            ori_img_size=ImageHelper.get_size(ori_img_bgr),
        ...
        with torch.no_grad():
            ...
            test_indices_and_rois, test_roi_locs, test_roi_scores, test_rois_num = (
                test_group
            )
        ...
        json_dict = self.__get_info_tree(
            batch_detections[0], ori_img_bgr, scale=scale
        )
    ...
    @staticmethod
    def decode(
        roi_locs, roi_scores, indices_and_rois, test_rois_num, configer, metas
    ):
        indices_and_rois = indices_and_rois
        num_classes = configer.get("data", "num_classes")
        mean = torch.Tensor(configer.get("roi", "loc_normalize_mean")).repeat(
            num_classes
        )[None]
        std = torch.Tensor(configer.get("roi", "loc_normalize_std")).repeat(
            num_classes
        )[None]
        ...
        cls_label = (
            torch.LongTensor([i for i in range(num_classes)])
            .contiguous()
            .view(1, num_classes)
            .repeat(indices_and_rois.size(0), 1)
            .to(roi_locs.device)
        )
        ...
        for i in range(test_rois_num.size(0)):
            ...
            tmp_dst_bbox = dst_bbox[
                start_index : start_index + test_rois_num[i]
            ]
            # clip bounding box
            ...
            tmp_cls_prob = cls_prob[
                start_index : start_index + test_rois_num[i]
            ]
            tmp_cls_label = cls_label[
                start_index : start_index + test_rois_num[i]
            ]
            start_index += test_rois_num[i]

            mask = (tmp_cls_prob > configer.get("res", "val_conf_thre")) & (
                tmp_cls_label > 0
            )
            ...
            tmp_cls_label = (
                tmp_cls_label[mask].contiguous().view(-1).unsqueeze(1)
            )
    ...
    def debug(self, vis_dir):
        ...
        for i, data_dict in enumerate(self.det_data_loader.get_trainloader()):
            ...
            self.det_visualizer.vis_rois(
                data_dict["img"], sample_rois[gt_roi_labels > 0]
            )
            ...
            gt_cls_roi_locs[
                torch.arange(0, sample_rois.size(0)).long(),
                gt_roi_labels.long(),
            ] = gt_roi_locs
            ...
            for j in range(data_dict["img"].size(0)):
                ...
                json_dict = self.__get_info_tree(
                    batch_detections[j], ori_img_bgr
                )
                image_canvas = self.det_parser.draw_bboxes(
                    ori_img_bgr.copy(),
                    json_dict,
