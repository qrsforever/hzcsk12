...
class YOLOv3Test(object):
    ...
    def __test_img(self, image_path, json_path, raw_path, vis_path):
        ...
        inputs = self.blob_helper.make_input(
            img, input_size=self.configer.get("data", "input_size"), scale=1.0
        )
    ...
    def decode(batch_pred_bboxes, configer, input_size):
        ...
        box_corner[:, :, 0] = (
            batch_pred_bboxes[:, :, 0] - batch_pred_bboxes[:, :, 2] / 2
        )
        box_corner[:, :, 1] = (
            batch_pred_bboxes[:, :, 1] - batch_pred_bboxes[:, :, 3] / 2
        )
        box_corner[:, :, 2] = (
            batch_pred_bboxes[:, :, 0] + batch_pred_bboxes[:, :, 2] / 2
        )
        ...
        for image_i, image_pred in enumerate(batch_pred_bboxes):
            ...
            conf_mask = (
                image_pred[:, 4] > configer.get("res", "val_conf_thre")
            ).squeeze()
            ...
            output[image_i] = DetHelper.cls_nms(
                detections,
    ...
    def debug(self, vis_dir):
        ...
        for i, data_dict in enumerate(self.det_data_loader.get_trainloader()):
            ...
            targets, _, _ = self.yolo_target_generator(
            ...
            for f_index, anchors in enumerate(anchors_list):
                feat_stride = self.configer.get("network", "stride_list")[
                    f_index
                ]
                fm_size = [
            ...
            batch_detections = self.decode(
                self.yolo_detection_layer(output_list)[2],
                self.configer,
                input_size,
            )
