...
class FasterRCNN(object):
    ...
    def _get_parameters(self):
        ...
        params = [
            ...
            {
                "params": lr_2,
                "lr": self.configer.get("solver", "lr")["base_lr"] * 2.0,
                "weight_decay": 0,
            },
        ]
        return params
    ...
    def train(self):
        ...
        for i, data_dict in enumerate(self.train_loader):
            ...
            self.train_losses.update(
                loss.item(), len(DCHelper.tolist(data_dict["meta"]))
            )

            self.optimizer.zero_grad()
            loss.backward()
            RunnerHelper.clip_grad(self.det_net, 10.0)
            self.optimizer.step()
        ...
        for idx, detections in enumerate(batch_detections):
            ...
            if detections is not None:
                for x1, y1, x2, y2, conf, cls_pred in detections:
                    ...
                    object_list.append(
                        [xmin, ymin, xmax, ymax, cls_pred, float("%.2f" % cf)]
                    )
