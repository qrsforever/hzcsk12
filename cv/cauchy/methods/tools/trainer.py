#!/usr/bin/env python
# -*- coding:utf-8 -*-
# setup optimizer and learning rate update schema
...
class Trainer(object):
    ...
    def init(net_params, solver_dict=None):
        ...
        if lr_params["lr_policy"] == "step":
            ...
            if lr_params["metric"] == "epoch":
                lambda_poly = lambda epoch: pow(
                    (1.0 - epoch / solver_dict["max_epoch"]), 0.9
                )
            else:
                assert lr_params["metric"] == "iters"
                lambda_poly = lambda epoch: pow(
                    (1.0 - epoch / solver_dict["max_iters"]), 0.9
                )
            ...
            if lr_params["metric"] == "epoch":
                lambda_linear = lambda epoch: 1.0 - (
                    epoch / solver_dict["max_epoch"]
                )
            else:
                assert lr_params["metric"] == "iters"
                lambda_linear = lambda epoch: 1.0 - (
                    epoch / solver_dict["max_iters"]
                )

            scheduler = lr_scheduler.LambdaLR(
                optimizer, lr_lambda=lambda_linear
            )
        ...
        elif lr_params["lr_policy"] == "annealing":
            scheduler = lr_scheduler.CosineAnnealingLR(
                optimizer, float(solver_dict["max_epoch"]), lr_params["lr_min"]
            )
    ...
    def update(runner, backbone_list=(), solver_dict=None):
        if (
            "is_warm" not in solver_dict["lr"]
            or not solver_dict["lr"]["is_warm"]
        ):
        ...
        if runner.runner_state["iters"] < warm_params["warm_iters"]:
            if warm_params["freeze_backbone"]:
                ...
                lr_ratio = (runner.runner_state["iters"] + 1) / warm_params[
                    "warm_iters"
                ]

                base_lr_list = runner.scheduler.get_lr()
                for param_group, base_lr in zip(
                    runner.optimizer.param_groups, base_lr_list
                ):
            ...
            try:
                ...
                nbb_lr = (
                    solver_dict["lr"]["base_lr"] * solver_dict["lr"]["nbb_mult"]
                )
