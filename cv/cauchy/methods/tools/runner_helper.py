...
from cauchy.extensions.tools.parallel.data_parallel import DataParallelModel
...
class RunnerHelper(object):
    ...
    def _make_parallel(runner, net):
        if (
            ...
            or len(range(torch.cuda.device_count())) == 1
        ):
            runner.configer.update(["network", "gathered"], True)

        return DataParallelModel(
            net, gather_=runner.configer.get("network", "gathered")
        )
    ...
    def load_net(runner, net, model_path=None):
        ...
        if (
            ...
            or runner.configer.get("network", "resume") is not None
        ):
            ...
            try:
                FileHelper.check_file_exist(resume_path)
    ...
    @staticmethod
    def save_net(
        runner, net, performance=None, val_loss=None, iters=None, epoch=None
    ):
        state = {
        ...
        if runner.configer.get("network", "checkpoints_root") is None:
            ...
            checkpoints_dir = os.path.join(
                runner.configer.get("project_dir"),
                runner.configer.get("network", "checkpoints_root"),
                runner.configer.get("network", "checkpoints_dir"),
            )
        ...
        if iters is not None:
            if iters - runner.runner_state["last_iters"] >= runner.configer.get(
                "solver", "save_iters"
            ):
                latest_name = "{}_iters{}.pth".format(
                    runner.configer.get("network", "checkpoints_name"), iters
                )
                torch.save(state, os.path.join(checkpoints_dir, latest_name))
                runner.runner_state["last_iters"] = iters

        if epoch is not None:
            if epoch - runner.runner_state["last_epoch"] >= runner.configer.get(
                "solver", "save_epoch"
            ):
                latest_name = "{}_epoch{}.pth".format(
                    runner.configer.get("network", "checkpoints_name"), epoch
                )
                torch.save(state, os.path.join(checkpoints_dir, latest_name))
    ...
    def freeze_bn(net, syncbn=False):
        for m in net.modules():
            ...
            if syncbn:
                from extensions.ops.syncbn.module import (
                    BatchNorm2d,
                    BatchNorm1d,
                )
    ...
    @staticmethod
    def clip_grad(net, max_grad=10.0):
