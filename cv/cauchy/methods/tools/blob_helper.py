...
from cauchy.datasets.tools.transforms import DeNormalize, ToTensor, Normalize
...
class BlobHelper(object):
    ...
    def make_input_batch(self, image_list, input_size=None, scale=1.0):
        ...
        for image in image_list:
            input_list.append(
                self.make_input(image, input_size=input_size, scale=scale)
            )
    ...
    def make_input(
        ...
        if (
            ...
            and min_side_length is None
            and max_side_length is None
        ):
            ...
            input_size is None
            and min_side_length is not None
            and max_side_length is None
        ):
            width, height = ImageHelper.get_size(image)
            ...
            input_size is None
            and min_side_length is None
            and max_side_length is not None
        ):
            width, height = ImageHelper.get_size(image)
        ...
        image = ImageHelper.resize(
            image,
            (int(in_width * scale), int(in_height * scale)),
            interpolation="cubic",
        )
        img_tensor = ToTensor()(image)
        img_tensor = Normalize(
            div_value=self.configer.get("data", "normalize")["div_value"],
            mean=self.configer.get("data", "normalize")["mean"],
            std=self.configer.get("data", "normalize")["std"],
        )(img_tensor)
