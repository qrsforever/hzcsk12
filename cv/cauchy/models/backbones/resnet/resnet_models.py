...
class BasicBlock(nn.Module):
    ...
    def __init__(
        self, inplanes, planes, stride=1, downsample=None, bn_type=None
    ):
...
class ResNet(nn.Module):
    def __init__(
        self, block, layers, num_classes=1000, deep_base=False, bn_type=None
    ):
        ...
        self.layer2 = self._make_layer(
            block, 128, layers[1], stride=2, bn_type=bn_type
        )
        self.layer3 = self._make_layer(
            block, 256, layers[2], stride=2, bn_type=bn_type
        )
        ...
        for m in self.modules():
            if isinstance(m, nn.Conv2d):
                ...
                m, ModuleHelper.BatchNorm2d(bn_type=bn_type, ret_cls=True)
            ):
    ...
    def _make_layer(self, block, planes, blocks, stride=1, bn_type=None):
        ...
        layers = []
        layers.append(
            block(self.inplanes, planes, stride, downsample, bn_type=bn_type)
        )
