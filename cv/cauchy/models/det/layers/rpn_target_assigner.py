...
class RPNTargetAssigner(object):
    ...
    def __call__(self, feat_list, gt_bboxes, meta):
        ...
        for i in range(len(gt_bboxes)):
            ...
            label = (
                torch.ones((default_boxes.size(0),)).mul_(-1).long().to(device)
            )
            ...
            if gt_bboxes[i].numel() > 0:
                ...
                pos_index = (
                    (label == 1).nonzero().contiguous().view(-1).cpu().numpy()
                )
                ...
                neg_index = (
                    (label == 0).nonzero().contiguous().view(-1).cpu().numpy()
                )
                ...
                cxcy = (boxes[:, :2] + boxes[:, 2:]) / 2 - default_boxes[
                    :, :2
                ]  # [8732,2]
                cxcy /= default_boxes[:, 2:]
                wh = (boxes[:, 2:] - boxes[:, :2]) / default_boxes[
                    :, 2:
                ]  # [8732,2]
                wh = torch.log(wh)
                ...
                n_neg = n_sample // 2
                neg_index = (
                    (label == -1).nonzero().contiguous().view(-1).numpy()
                )
                if len(neg_index) > n_neg:
                    disable_index = np.random.choice(
                        neg_index, size=n_neg, replace=False
                    )
                    label[disable_index] = 0

            ret_label = (
                torch.ones((anchor_boxes.size(0),), dtype=torch.long)
                .mul_(-1)
                .to(device)
            )
