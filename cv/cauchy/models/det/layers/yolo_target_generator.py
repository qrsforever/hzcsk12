...
class YOLOTargetGenerator(object):
    ...
    def __call__(self, feat_list, batch_gt_bboxes, batch_gt_labels, input_size):
        ...
        for i, ori_anchors in enumerate(
            self.configer.get("anchor", "anchors_list")
        ):
            in_h, in_w = feat_list[i].size()[2:]
            w_fm_stride, h_fm_stride = (
                input_size[0] / in_w,
                input_size[1] / in_h,
            )
            anchors = [
                (a_w / w_fm_stride, a_h / h_fm_stride)
            ...
            for b in range(batch_size):
                for t in range(batch_gt_bboxes[b].size(0)):
                    ...
                    gx = (
                        (batch_gt_bboxes[b][t, 0] + batch_gt_bboxes[b][t, 2])
                        / (2.0 * input_size[0])
                        * in_w
                    )
                    gy = (
                        (batch_gt_bboxes[b][t, 1] + batch_gt_bboxes[b][t, 3])
                        / (2.0 * input_size[1])
                        * in_h
                    )
                    gw = (
                        (batch_gt_bboxes[b][t, 2] - batch_gt_bboxes[b][t, 0])
                        / input_size[0]
                        * in_w
                    )
                    gh = (
                        (batch_gt_bboxes[b][t, 3] - batch_gt_bboxes[b][t, 1])
                        / input_size[1]
                        * in_h
                    )
                    ...
                    if anch_ious[0, best_n] < self.configer.get(
                        "anchor", "iou_threshold"
                    ):
                        continue
            ...
            tcls = tcls.view(
                batch_size, -1, self.configer.get("data", "num_classes")
            )
