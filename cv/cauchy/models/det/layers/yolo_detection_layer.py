...
class YOLODetectionLayer(object):
    ...
    def __call__(self, layer_out_list):
        ...
        for i in range(len(layer_out_list)):
            ...
            anchors = [
                (a[0] / feat_stride, a[1] / feat_stride) for a in in_anchors
            ]

            layer_out = layer_out_list[i].view(
                batch_size, num_anchors * bbox_attrs, grid_size_h * grid_size_w
            )
            layer_out = layer_out.contiguous().view(
                batch_size, num_anchors, bbox_attrs, grid_size_h * grid_size_w
            )
            layer_out = (
                layer_out.permute(0, 1, 3, 2)
                .contiguous()
                .view(batch_size, -1, bbox_attrs)
            )
            ...
            if self.configer.get("phase") != "debug":
                ...
                layer_out[:, :, 5 : 5 + num_classes] = torch.sigmoid(
                    (layer_out[:, :, 5 : 5 + num_classes])
                )
            ...
            x_y_offset = (
                torch.cat((x_offset, y_offset), 1).contiguous().view(1, -1, 2)
            )
            x_y_offset = (
                x_y_offset.repeat(num_anchors, 1, 1).view(-1, 2).unsqueeze(0)
            )
            ...
            anchors = (
                anchors.contiguous()
                .view(3, 1, 2)
                .repeat(1, grid_size_h * grid_size_w, 1)
                .contiguous()
                .view(-1, 2)
                .unsqueeze(0)
            )
        ...
        return (
            layer_out_list,
            torch.cat(prediction_list, 1),
            torch.cat(detect_list, 1),
        )
