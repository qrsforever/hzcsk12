    ...
    def __call__(self, feat_list, gt_bboxes, gt_labels, input_size):
        ...
        for i in range(len(gt_bboxes)):
            if gt_bboxes[i] is None or len(gt_bboxes[i]) == 0:
                ...
                variances = [0.1, 0.2]
                cxcy = (boxes[:, :2] + boxes[:, 2:]) / 2 - anchor_boxes[
                    :, :2
                ]  # [8732,2]
                cxcy /= variances[0] * anchor_boxes[:, 2:]
                wh = (boxes[:, 2:] - boxes[:, :2]) / anchor_boxes[
                    :, 2:
                ]  # [8732,2]
                ...
                conf = (
                    1 + gt_labels[i][max_idx]
                )  # [8732,], background class = 0
                ...
                if self.configer.get("anchor", "anchor_method") == "retina":
                    ...
                    conf[
                        prior_box_iou
                        < self.configer.get("anchor", "iou_threshold") - 0.1
                    ] = 0
                else:
                    conf[
                        prior_box_iou
