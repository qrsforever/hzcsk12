...
from cauchy.utils.helpers.mask_helper import MaskHelper
...
class FRROISampler(object):
    ...
    def __call__(
        self, indices_and_rois, gt_bboxes, gt_labels, meta, gt_polygons=None
    ):
        n_sample = self.configer.get("roi", "sampler")["n_sample"]
        pos_iou_thresh = self.configer.get("roi", "sampler")["pos_iou_thresh"]
        neg_iou_thresh_hi = self.configer.get("roi", "sampler")[
            "neg_iou_thresh_hi"
        ]
        neg_iou_thresh_lo = self.configer.get("roi", "sampler")[
            "neg_iou_thresh_lo"
        ]
        ...
        for i in range(len(gt_bboxes)):
            ...
            if temp_gt_bboxes.numel() == 0:
                ...
                sample_roi = (
                    ...
                    .float()
                    .to(indices_and_rois.device)
                )
                sample_roi[0, 2:] = roi_size
                gt_roi_loc = (
                    torch.zeros((1, 4), requires_grad=True)
                    .float()
                    .to(sample_roi.device)
                )
                gt_roi_label = (
                ...
                if self.configer.get("phase") == "debug":
                    ...
                    if indices_and_rois.numel() == 0:
                        ...
                        rois = torch.cat(
                            (
                                indices_and_rois[indices_and_rois[:, 0] == i][
                                    :, 1:
                                ],
                                temp_gt_bboxes,
                            ),
                ...
                neg_index = np.where(
                    (max_iou < neg_iou_thresh_hi)
                    & (max_iou >= neg_iou_thresh_lo)
                )[0]
                ...
                if gt_polygons is not None:
                    ...
                    target_size = [
                        self.configer.get("roi", "pooled_width"),
                        self.configer.get("roi", "pooled_height"),
                    ]
                    for roi_index in range(pos_roi_per_this_image):
                        ...
                        mask = MaskHelper.polys2mask_wrt_box(
                            roi_polygons, roi, target_size
                        )
                        mask = torch.from_numpy(mask).to(
                            indices_and_rois.device
                        )
            ...
            batch_index = i * torch.ones((len(sample_roi),)).to(
                sample_roi.device
            )
            sample_roi = torch.cat(
                [batch_index[:, None], sample_roi], dim=1
            ).contiguous()
