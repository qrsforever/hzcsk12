...
class FRROIGenerator(object):
    ...
    def __call__(self, feat_list, loc, score, n_pre_nms, n_post_nms, meta):
        ...
        default_boxes = (
            anchors.unsqueeze(0).repeat(loc.size(0), 1, 1).to(device)
        )
        ...
        for i in range(loc.size(0)):
            ...
            tmp_dst_bbox[:, 0::2] = tmp_dst_bbox[:, 0::2].clamp_(
                min=0, max=meta[i]["border_size"][0] - 1
            )
            tmp_dst_bbox[:, 1::2] = tmp_dst_bbox[:, 1::2].clamp_(
                min=0, max=meta[i]["border_size"][1] - 1
            )
            ...
            keep = (hs >= meta[i]["img_scale"] * min_size) & (
                ws >= meta[i]["img_scale"] * min_size
            )
            ...
            _, keep = nms(
                torch.cat((rois, tmp_scores.unsqueeze(1)), 1),
        ...
        if rois.numel() == 0:
            ...
            indices_and_rois = torch.cat(
                [roi_indices.unsqueeze(1).to(device), rois.to(device)], dim=1
            ).contiguous()
