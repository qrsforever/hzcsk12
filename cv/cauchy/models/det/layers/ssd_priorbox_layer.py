...
class SSDPriorBoxLayer(object):
    ...
    def __call__(self, feat_list, input_size):
        ...
        for i in range(len(feat_list)):
            ...
            boxes = []
            stride_offset_w, stride_offset_h = (
                0.5 * stride_w_list[i],
                0.5 * stride_h_list[i],
            )
            if self.configer.get("anchor", "anchor_method") == "ssd":
                ...
                extra_s = math.sqrt(
                    self.configer.get("anchor", "cur_anchor_sizes")[i]
                    * self.configer.get("anchor", "cur_anchor_sizes")[i + 1]
                )

                boxes.append(
                    (stride_offset_w, stride_offset_h, extra_s, extra_s)
                )
                ...
                Log.error(
                    "Anchor Method {} not valid.".format(
                        self.configer.get("anchor", "anchor_method")
                    )
                )
                exit(1)
            ...
            assert (
                anchor_bases.size(0)
                == self.configer.get("anchor", "num_anchor_list")[i]
            )
            anchors = (
                anchor_bases.contiguous()
                .view(1, -1, 4)
                .repeat(fm_h * fm_w, 1, 1)
                .contiguous()
                .view(-1, 4)
            )
            ...
            x_y_offset = (
                x_y_offset.repeat(
                    1, self.configer.get("anchor", "num_anchor_list")[i], 1
                )
                .contiguous()
                .view(-1, 2)
            )
        ...
        if self.clip:
            anchor_boxes[:, 0::2].clamp_(min=0.0, max=img_w - 1)
            anchor_boxes[:, 1::2].clamp_(min=0.0, max=img_h - 1)
