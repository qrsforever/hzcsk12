...
class SSDFocalLoss(nn.Module):
    ...
    def focal_loss(self, x, y):
        ...
        logit = F.softmax(x)
        logit = logit.clamp(1e-7, 1.0 - 1e-7)
...
class SSDMultiBoxLoss(nn.Module):
    ...
    def forward(self, outputs, *targets, **kwargs):
        ...
        conf_loss = self._cross_entropy_loss(
            conf_preds.view(-1, self.num_classes), conf_targets.view(-1)
        )  # [N*8732,]
        ...
        conf_loss = F.cross_entropy(
            preds, targets, reduction="sum", ignore_index=-1
        )
...
class YOLOv3Loss(nn.Module):
    ...
    def forward(self, prediction, targets, objmask, noobjmask):
        ...
        loss_cls = self.bce_loss(
            pred_cls * objmask.unsqueeze(2), tcls * objmask.unsqueeze(2)
        )
        loss_cls = loss_cls / obj_cnt

        #  total loss = losses * weight
        loss = (
            loss_coord * self.configer.get("loss", "loss_weights")["coord_loss"]
...
class FRLocLoss(nn.Module):
    ...
    def forward(self, pred_locs, gt_locs, gt_labels, sigma):
        ...
        loc_loss /= max(
            (gt_labels.float() >= 0).sum().float(), 1e-9
    ...
    def smooth_l1_loss(x, t, in_weight, sigma):
        ...
        y = flag * (sigma2 / 2.0) * (diff ** 2) + (1 - flag) * (
            abs_diff - 0.5 / sigma2
        )
        return y.sum()
...
class FRLoss(nn.Module):
    ...
    def forward(self, pred_group, target_group):
        ...
        pred_rpn_locs, pred_rpn_scores, pred_roi_cls_locs, pred_roi_scores = (
            pred_group
        )
        gt_rpn_locs, gt_rpn_labels, gt_roi_cls_locs, gt_roi_labels = (
            target_group
        )
        gt_rpn_labels = gt_rpn_labels.contiguous().view(-1)
        pred_rpn_scores = pred_rpn_scores.contiguous().view(-1, 2)
        rpn_loc_loss = self.fr_loc_loss(
            pred_rpn_locs,
            gt_rpn_locs,
            gt_rpn_labels,
            self.configer.get("loss", "params")["rpn_sigma"],
        )

        # NOTE: default value of ignore_index is -100 ...
        rpn_cls_loss = F.cross_entropy(
            pred_rpn_scores, gt_rpn_labels, ignore_index=-1
        )

        roi_loc_loss = self.fr_loc_loss(
            pred_roi_cls_locs,
            gt_roi_cls_locs,
            gt_roi_labels,
            self.configer.get("loss", "params")["roi_sigma"],
        )
        roi_cls_loss = F.cross_entropy(
            pred_roi_scores, gt_roi_labels, ignore_index=-1
        )
        ...
        rpn_loss = (rpn_loc_loss + rpn_cls_loss) * self.configer.get(
            "loss", "loss_weights"
        )["rpn_loss"]
        roi_loss = (roi_loc_loss + roi_cls_loss) * self.configer.get(
            "loss", "loss_weights"
        )["roi_loss"]
