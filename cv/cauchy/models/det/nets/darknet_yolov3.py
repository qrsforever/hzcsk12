...
class DarkNetYolov3(nn.Module):
    def __init__(self, configer):
        ...
        self.yolov3_head = Yolov3Head(
            configer, out_filters=self.backbone.num_features
        )
...
class Yolov3Head(nn.Module):
    def __init__(self, configer, out_filters=None):
        ...
        final_out_filter0 = len(
            self.configer.get("anchor", "anchors_list")[0]
        ) * (5 + self.num_classes)
        ...
        final_out_filter1 = len(
            self.configer.get("anchor", "anchors_list")[1]
        ) * (5 + self.num_classes)

        self.embedding1_cbl = self._make_cbl(512, 256, 1)
        self.embedding1 = self._make_embedding(
            [256, 512], _out_filters[-2] + 256
        )
        ...
        final_out_filter2 = len(
            self.configer.get("anchor", "anchors_list")[2]
        ) * (5 + self.num_classes)

        self.embedding2_cbl = self._make_cbl(256, 128, 1)
        self.embedding2 = self._make_embedding(
            [128, 256], _out_filters[-3] + 128
        )
    ...
    def forward(self, tuple_features):
        ...
        x1_in = torch.cat(
            [
                F.interpolate(x1_in, scale_factor=2, mode="nearest"),
                tuple_features[-2],
            ],
            1,
        )
        ...
        x2_in = torch.cat(
            [
                F.interpolate(x2_in, scale_factor=2, mode="nearest"),
                tuple_features[-3],
            ],
            1,
        )
