...
def reorg(x, stride_h=2, stride_w=2):
    ...
    if 1:
        x = (
            x.view(batch_size, channels, _height, stride_h, _width, stride_w)
            .transpose(3, 4)
            .contiguous()
        )
        x = (
            x.view(batch_size, channels, _height * _width, stride_h * stride_w)
            .transpose(2, 3)
            .contiguous()
        )
        x = (
            x.view(batch_size, channels, stride_h * stride_w, _height, _width)
            .transpose(1, 2)
            .contiguous()
        )
        ...
        x = x.permute(
            0, 1, 3, 5, 2, 4
        )  # batch_size, channels, stride, stride, _height, _width
        x = x.contiguous()
        x = x.view(batch_size, -1, _height, _width)
    return x
...
class DarkNetYolov2(nn.Module):
    def __init__(self, configer):
        ...
        # linear
        out_channels = self.configer.get("details", "num_anchors") * (
            self.configer.get("data", "num_classes") + 5
        )
