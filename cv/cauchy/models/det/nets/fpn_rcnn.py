...
from cauchy.models.backbones.backbone_selector import BackboneSelector
    ...
    def forward(self, *inputs):
        ...
        * :math:`L` is the number of classes excluding the background.
        ...
        if self.configer.get("phase") == "test" and not self.training:
            ...
            indices_and_rois, test_rois_num = self.roi(
                rpn_locs,
                rpn_scores,
            ...
            roi_cls_locs, roi_scores = self.head(
                mrcnn_feature_maps, indices_and_rois, input_size
            )
            ...
            feat_list, rpn_locs, rpn_scores = self.rpn(rpn_feature_maps)
            test_indices_and_rois, test_rois_num = self.roi(
                feat_list,
                rpn_locs,
                rpn_scores,
                self.configer.get("rpn", "n_test_pre_nms"),
            ...
            test_roi_cls_locs, test_roi_scores = self.head(
                mrcnn_feature_maps, test_indices_and_rois, input_size
            )
            ...
            train_indices_and_rois, _ = self.roi(
                feat_list,
                rpn_locs,
                rpn_scores,
            ...
            sample_roi_locs, sample_roi_scores = self.head(
                mrcnn_feature_maps, sample_rois, input_size
            )
    ...
    def __init__(self, configer):
        ...
        if self.configer.get("roi", "class_agnostic"):
            ...
            self.cls_loc = nn.Linear(
                1024, 4 * self.configer.get("data", "num_classes")
            )

        from extensions.roialign.module import RoIAlign2D

        self.roi_align = RoIAlign2D(
            pooled_height=int(self.configer.get("roi", "pooled_height")),
