...
from cauchy.models.det.loss.det_modules import FRLoss
...
from cauchy.models.det.layers.rpn_detection_layer import RPNDetectionLayer
...
class VGGModel(object):
    ...
    def __call__(self):
        ...
        if self.configer.get("network", "pretrained") is not None:
            Log.info(
                "Loading pretrained model: {}".format(
    ...
    def forward(self, data_dict):
        ...
        * :math:`L` is the number of classes excluding the background.
        ...
        if self.configer.get("phase") == "test" and not self.training:
            ...
            indices_and_rois, test_rois_num = self.roi_generator(
                feat_list,
                rpn_locs,
                rpn_scores,
            ...
            roi_cls_locs, roi_scores = self.bbox_head(
                x, indices_and_rois, data_dict["meta"]
            )
            ...
            x = self.backbone(data_dict["img"])
            feat_list, rpn_locs, rpn_scores = self.rpn(x)
            gt_rpn_locs, gt_rpn_labels = self.rpn_target_assigner(
                feat_list, data_dict["bboxes"], data_dict["meta"]
            )

            test_indices_and_rois, test_rois_num = self.roi_generator(
                feat_list,
                rpn_locs,
                rpn_scores,
                self.configer.get("rpn", "n_test_pre_nms"),
            ...
            test_roi_cls_locs, test_roi_scores = self.bbox_head(
                x, test_indices_and_rois, data_dict["meta"]
            )
            test_group = [
            ...
            train_indices_and_rois, _ = self.roi_generator(
                feat_list,
                rpn_locs,
                rpn_scores,
            ...
            sample_rois, gt_roi_bboxes, gt_roi_labels = self.roi_sampler(
                train_indices_and_rois,
                data_dict["bboxes"],
                data_dict["labels"],
                data_dict["meta"],
            )

            sample_roi_locs, sample_roi_scores = self.bbox_head(
                x, sample_rois, data_dict["meta"]
            )
            sample_roi_locs = sample_roi_locs.contiguous().view(
                -1, self.configer.get("data", "num_classes"), 4
            )
            sample_roi_locs = (
                sample_roi_locs[
                    torch.arange(0, sample_roi_locs.size()[0])
                    .long()
                    .to(sample_roi_locs.device),
                    gt_roi_labels.long().to(sample_roi_locs.device),
                ]
                .contiguous()
                .view(-1, 4)
            )
...
class BBoxHead(nn.Module):
    ...
    def __init__(self, configer, classifier):
        ...
        self.cls_loc = nn.Linear(
            4096, self.configer.get("data", "num_classes") * 4
        )
        ...
        self.roi_pool = RoIPool(
            out_size=tuple(self.configer.get("roi", "pooled_hw")),
            spatial_scale=1.0
            / float(self.configer.get("roi", "spatial_stride")),
        )
