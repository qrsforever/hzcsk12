...
class ProposalLayer(object):
    ...
    def __find_top_k_subsets(self, peak, subsets, sim_array):
        ...
        for key, value in sorted(
            score_dict.items(), lambda x, y: cmp(x[1], y[1]), reverse=True
        ):
    ...
    def __get_subsets(self, all_peaks, sim_array):
        ...
        for i in range(self.configer.get("num_keypoints")):
            subsets_new = -1 * np.ones(
                (0, self.configer.get("num_keypoints") + 2)
            )
            update_index = list()
    ...
    def __cluster(self, inputs, mask):
        ...
        for i in range(self.configer.get("batch_size")):
            heatmap = inputs[i, :, :, :].view(
                self.configer.get("num_keypoints"),
    ...
    def __make_features_labels(
        self, inputs, proposals_list, candidates_list, kpts
    ):
            ...
            for proposal in proposals:
                ...
                for j in range(self.configer.get("num_keypoints")):
                    vec = vecmap[
                        j,
                        :,
                        candidate[proposal[i]][1],
                        candidate[proposal[i]][0],
                    ]
                    feature[j, :] = vec
        ...
        labels = Variable(
            torch.zeros(proposal_count, self.configer.get("num_keypoints")),
            requires_grad=True,
        ).cuda()
    ...
    def __get_label(self, proposal, kpts, candidates):
        label = Variable(
            torch.zeros(self.configer.get("num_keypoints")), require_grad=True
        ).cuda()
        ...
        for i in range(len(kpts)):
            ...
            visiable_count = 0
            label_temp = Variable(
                torch.zeros(self.configer.get("num_keypoints")),
                require_grad=True,
            ).cuda()
            for j in range(self.configer.get("num_keypoints")):
                if kpts[i][j][2] > 1:
    ...
    def forward(self, inputs, kpts, mask, is_label=True):
        ...
        features, labels = self.__make_features_labels(
            proposals_list, candidates_list, kpts
        )
