        ...
        for k in range(len(self.configer.get("coco", "limb_seq"))):
            ...
            if lena != 0 and lenb != 0:
                ...
                for i in range(lena):
                    for j in range(lenb):
                        vec1 = vecmap[
                            self.configer.get("coco", "limb_seq")[k][0],
                            canda[i][1],
                            canda[i][0],
                        ]
                        vec2 = vecmap[
                            self.configer.get("coco", "limb_seq")[k][1],
                            candb[j][1],
                            candb[j][0],
                        ]
                        score_with_dist_prior = 1.0 - np.sqrt(
                            ((vec1 - vec2) * (vec1 - vec2)).sum()
                        )
                        print(score_with_dist_prior)

                        if score_with_dist_prior > self.configer.get(
                            "vis", "limb_threshold"
                        ):
                ...
                connection_candidate = sorted(
                    connection_candidate, key=lambda x: x[3], reverse=True
                )
    ...
    def __get_proposals(self, connection_all, candidate, special_k):
        ...
        for k in range(len(self.configer.get("coco", "limb_seq"))):
            if k not in special_k:
                ...
                indexa, indexb = (
                    np.array(self.configer.get("coco", "limb_seq")[k]) - 1
                )
                ...
                for i in range(len(connection_all[k])):
                    ...
                    if found == 1:
                        ...
                        found == 2
                    ):  # if found equals to 2 and disjoint, merge them
                        j1, j2 = subset_idx
                        membership = (
