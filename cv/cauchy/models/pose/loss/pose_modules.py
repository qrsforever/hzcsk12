...
class OPMseLoss(nn.Module):
    def __init__(self, configer):
        ...
        if self.configer.exists(
            "loss", "params"
        ) and "mse_reduction" in self.configer.get("loss", "params"):
        ...
        if isinstance(inputs, list):
            if weights is not None:
                for i in range(len(inputs)):
                    if mask is not None:
                        loss += weights[i] * self.mse_loss(
                            inputs[i] * mask, targets
                        )
                    else:
...
class PartLoss(nn.Module):
    ...
    def forward(self, inputs, targets, mask=None):
        inputs = inputs.view(
            inputs.size(0), -1, 6, inputs.size(2), inputs.size(3)
        )
        targets = targets.view(
            targets.size(0), -1, 6, targets.size(2), targets.size(3)
        )
        paf_loss = self.mse_loss(
            inputs[:, :, 0:2, :, :], targets[:, :, 0:2, :, :]
        )
        part_loss = self.mse_loss(
            inputs[:, :, 2:6, :, :], targets[:, :, 2:6, :, :]
        )
...
class CapsuleLoss(nn.Module):
    def __init__(self, configer):
        ...
        self.mse_loss = nn.MSELoss(
            reduction=self.configer.get("capsule_loss", "reduction")
        )
...
class EmbeddingLoss(nn.Module):
    ...
    def forward(self, inputs, tags, numH, sigma=0.1):
        ...
        h_tag_means = [
            [
                Variable(torch.zeros(self.l_vec), requires_grad=True).cuda()
                for h in range(numH[b].numpy()[0])
            ]
        ...
        for b in range(batch_size):
            for n in range(numH[b].numpy()[0]):
                ...
                for k in range(self.num_keypoints):
                    tag = inputs[b].masked_select(
                        tags[b][k].eq(n + 1).unsqueeze(0)
                    )
            ...
            for n1 in range(numH[b].numpy()[0]):
                for n2 in range(numH[b].numpy()[0]):
                    if n1 != n2:
                        loss_same = torch.exp(
                            -self.mse_loss(
                                h_tag_means[b][n1], h_tag_means[b][n2]
                            )
                            / sigma
                            / sigma
                        )
