...
class Cell(nn.Module):
    def __init__(
        self, steps, multiplier, cpp, cp, c, reduction, reduction_prev
    ):
        """
...
class Network(nn.Module):
    ...
    """
        ...
        for i in range(layers):
            ...
            cell = Cell(
                steps, multiplier, cpp, cp, c_curr, reduction, reduction_prev
            )
    ...
    def new(self):
        ...
        """
        model_new = Network(
            self.c, self.num_classes, self.layers, self.criterion
        ).cuda()
