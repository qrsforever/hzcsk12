...
class Arch:
    def __init__(self, model, configer):
        lr_params = configer.get("solver")
        self.momentum = lr_params["optim"]["sgd"][
            "arch_momentum"
        ]  # momentum for optimizer of theta
        self.wd = lr_params["optim"]["sgd"][
            "weight_decay"
        ]  # weight decay for optimizer of theta
        self.arch_wd = lr_params["optim"]["sgd"][
            "arch_weight_decay"
        ]  # weight decay for optimizer of theta
    ...
    """
        ...
        try:
            ...
            moment = concat(
                optimizer.state[v]["momentum_buffer"]
    ...
    """
        ...
        unrolled_model = self.comp_unrolled_model(
            x_train, target_train, eta, optimizer
        )
        # calculate loss on theta_pi
        unrolled_loss = unrolled_model.loss(x_valid, target_valid)

        loss_val = unrolled_loss.detach().cpu().numpy()
        ...
        implicit_grads = self.hessian_vector_product(
            vector, x_train, target_train
        )
