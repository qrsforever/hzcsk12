...
parser.add_argument(
    "--init_ch", type=int, default=36, help="num of init channels"
)
parser.add_argument(
...
def main():
    ...
    model = Network(
        args.init_ch, 10, args.layers, args.auxiliary, genotype
    ).cuda()
...
def train(train_queue, model, criterion, optimizer):
    ...
    for step, (x, target) in enumerate(train_queue):
        ...
        if step % args.report_freq == 0:
            logging.info(
                "train %03d %e %f %f", step, objs.avg, top1.avg, top5.avg
            )
