...
from cauchy.models.tools.module_helper import ModuleHelper
...
class _ConvBatchNormReluBlock(nn.Module):
    def __init__(
        ...
        self.bn = ModuleHelper.BatchNorm2d(bn_type=bn_type)(
...
class PPMBilinearDeepsup(nn.Module):
    def __init__(self, num_class=150, fc_dim=4096, bn_type=None):
        ...
        self.cbr_deepsup = _ConvBatchNormReluBlock(
            fc_dim // 2, fc_dim // 4, 3, 1, bn_type=bn_type
        )
...
class EmbedModule(nn.Module):
    def __init__(self, inchannels, bn_type):
        ...
        self.conv = nn.Sequential(
            nn.Conv2d(
                inchannels, inter_channels, kernel_size=1, padding=0, bias=False
            ),
            ...
            nn.ReLU(inplace=True),
            nn.Conv2d(
                inter_channels, inchannels, kernel_size=1, padding=0, bias=False
            ),
...
class EmbedNet(nn.Sequential):
    def __init__(self, configer):
        ...
        self.high_features1 = nn.Sequential(
            self.backbone.layer2, self.backbone.layer3
        )
        self.high_features2 = nn.Sequential(self.backbone.layer4)
        self.embed_conv = EmbedModule(
            1024, bn_type=self.configer.get("network", "bn_type")
        )
        self.decoder = PPMBilinearDeepsup(
