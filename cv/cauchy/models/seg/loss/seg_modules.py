...
class FSCELoss(nn.Module):
    ...
    def forward(self, inputs, *targets, weights=None, **kwargs):
        ...
        if isinstance(inputs, list):
            ...
            target = self._scale_target(
                targets[0], (inputs.size(2), inputs.size(3))
            )
...
class FSOhemCELoss(nn.Module):
    def __init__(self, configer):
        ...
        self.min_kept = max(
            1, self.configer.get("loss", "params")["ohem_minkeep"]
        )
        weight = None
        if self.configer.exists(
            "loss", "params"
    ...
    def forward(self, predict, target, **kwargs):
        ...
        sort_prob, sort_indices = (
            prob.contiguous().view(-1)[mask].contiguous().sort()
        )
...
class FSAuxOhemCELoss(nn.Module):
    ...
    def forward(self, inputs, targets, **kwargs):
        ...
        aux_targets = self._scale_target(
            targets, (aux_out.size(2), aux_out.size(3))
        )
        aux_loss = self.ce_loss(aux_out, aux_targets)
        loss = self.configer.get("loss", "loss_weights")["seg_loss"] * seg_loss
        loss = (
            loss
            + self.configer.get("loss", "loss_weights")["aux_loss"] * aux_loss
        )
        return loss
...
class FSAuxEncCELoss(nn.Module):
    ...
    def forward(self, outputs, targets, **kwargs):
        ...
        enc_loss = self.enc_loss(
            enc_out, aux_targets, self.configer.get("loss", "enc_size")
        )
        loss = (
            loss
            + self.configer.get("loss", "loss_weights")["enc_loss"] * enc_loss
        )
        return loss
...
class FSEncLoss(nn.Module):
    def __init__(self, configer):
        ...
        if self.configer.exists(
            "loss", "params"
        ) and "enc_weight" in self.configer.get("loss", "params"):
        ...
        if self.configer.exists(
            "loss", "params"
        ) and "enc_reduction" in self.configer.get("loss", "params"):
    ...
    def forward(self, preds, targets, grid_size=None, **kwargs):
        ...
        se_target = self._get_batch_label_vector(
            targets, self.configer.get("data", "num_classes"), grid_size
        ).type_as(preds)
    ...
    def _get_batch_label_vector(target_, num_classes, grid_size=None):
        ...
        if grid_size is not None:
            target = target.contiguous().view(
                b, h // grid_size, grid_size, w // grid_size, grid_size
            )
            target = (
                target.permute(0, 1, 3, 2, 4)
                .contiguous()
                .view(b * h * w // (grid_size ** 2), grid_size, grid_size)
            )
...
class FSEmbedLoss(nn.Module):
    ...
    def forward(self, inputs, targets, **kwargs):
        ...
        center_array = torch.zeros(
            (self.num_classes, inputs.size()[0]), requires_grad=True
        ).cuda()
        ...
        for i in range(self.num_classes):
            ...
            sim_center = (
                center.contiguous().view(1, -1).repeat(sim_input.size()[0], 1)
            )
            sim_loss = sim_loss + self.cosine_loss(
                sim_center, sim_input, sim_label
            )
        ...
        for i in range(self.num_classes):
            ...
            diff_loss = diff_loss + self.cosine_loss(
                center_array, center_dual, label
            )
