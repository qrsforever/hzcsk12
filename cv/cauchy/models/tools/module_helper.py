    ...
    def BNReLU(num_features, bn_type=None, **kwargs):
        if bn_type == "torchbn":
            return nn.Sequential(
                nn.BatchNorm2d(num_features, **kwargs), nn.ReLU()
            )
        elif bn_type == "syncbn":
            from cauchy.extensions.ops.syncbn.module import BatchNorm2d

            return nn.Sequential(BatchNorm2d(num_features, **kwargs), nn.ReLU())
        elif bn_type == "inplace_abn":
            from cauchy.extensions.ops.inplace_abn.bn import InPlaceABNSync
