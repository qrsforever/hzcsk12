...
from . import fbnet_builder as mbuilder, fbnet_modeldef as modeldef
import torch.nn as nn
from cauchy.models.rcnn.modeling import registry
from cauchy.models.rcnn.modeling.rpn import rpn
from cauchy.models.rcnn.modeling import poolers
...
def _get_head_stage(arch, head_name, blocks):
    ...
    ret = mbuilder.get_blocks(
        arch, stage_indices=head_stage, block_indices=blocks
    )
    return ret["stages"]
...
class FBNetROIHead(nn.Module):
    def __init__(
        ...
        self.head = nn.Sequential(
            OrderedDict([("blocks", blocks), ("last", last)])
        )
