...
from cauchy.models.rcnn.layers import (
    BatchNorm2d,
    Conv2d,
    FrozenBatchNorm2d,
    interpolate,
)
from cauchy.models.rcnn.layers.misc import _NewEmptyTensorOp
...
class Shift(nn.Module):
    def __init__(self, C, kernel_size, stride, padding):
        ...
        self.C = C
        kernel = torch.zeros(
            (C, 1, kernel_size, kernel_size), dtype=torch.float32
        )
        ch_idx = 0
...
def _get_upsample_op(stride):
    assert (
        ...
        or stride in [-1, -2, -4]
        or (
            isinstance(stride, tuple) and all(x in [-1, -2, -4] for x in stride)
        )
    )
...
def get_blocks(arch_def, stage_indices=None, block_indices=None):
    ...
    for block in arch_def["stages"]:
        keep = True
        if (
            stage_indices not in (None, [])
            and block["stage_idx"] not in stage_indices
        ):
            keep = False
        if (
            block_indices not in (None, [])
            and block["block_idx"] not in block_indices
        ):
            keep = False
        if keep:
