...
class Configer(object):
    ...
    def get(self, *key):
        if len(key) == 0:
            ...
            if (
                key[0] in self.params_root
                and key[1] in self.params_root[key[0]]
            ):
    ...
    def exists(self, *key):
        ...
        if len(key) == 2 and (
            key[0] in self.params_root and key[1] in self.params_root[key[0]]
        ):
            return True
    ...
    def add(self, key_tuple, value):
        if self.exists(*key_tuple):
            Log.error(
                "{} Key: {} existed!!!".format(self._get_caller(), key_tuple)
            )
            exit(1)
    ...
    def plus_one(self, *key):
        if not self.exists(*key):
            Log.error(
                "{} Key: {} not existed!!!".format(self._get_caller(), key)
            )
            exit(1)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--hypes",
        default="../../hypes/cls/flower/fc_vgg19_flower_cls.json",
        type=str,
        dest="hypes",
    ...
    parser.add_argument(
        ...
        type=str,
        dest="phase",
        help="The phase of Pose Estimator.",
    )
    parser.add_argument(
        "--gpu",
        default=[0, 1, 2, 3],
