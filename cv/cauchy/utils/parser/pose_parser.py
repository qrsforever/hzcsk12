...
class PoseParser(object):
    ...
    def parse_img_pose(self, image_file, json_file, mask_file=None):
        ...
        if mask_file is not None:
            ...
            image_canvas = cv2.addWeighted(
                image_canvas, 0.6, mask_canvas, 0.4, 0
            )
    ...
    def parse_dir_pose(self, image_dir, json_dir, mask_dir=None):
        ...
        for image_file in os.listdir(image_dir):
            ...
            Log.info(image_file)
            image_canvas = cv2.imread(
                os.path.join(image_dir, image_file)
            )  # B, G, R order.
            with open(
                os.path.join(json_dir, "{}.json".format(shotname)), "r"
            ) as json_stream:
                info_tree = json.load(json_stream)
                image_canvas = self.draw_points(image_canvas, info_tree)
                if self.configer.exists("details", "limb_seq"):
            ...
            if mask_dir is not None:
                mask_file = os.path.join(
                    mask_dir, "{}_vis.png".format(shotname)
                )
                mask_canvas = cv2.imread(mask_file)
                image_canvas = cv2.addWeighted(
                    image_canvas, 0.6, mask_canvas, 0.4, 0
    ...
    def draw_points(self, image_canvas, info_tree):
        for object in info_tree["objects"]:
            for i in range(self.configer.get("data", "num_kpts")):
                ...
                cv2.circle(
                    image_canvas,
                    (int(object["kpts"][i][0]), int(object["kpts"][i][1])),
    ...
    def link_points(self, image_canvas, info_tree):
        for object in info_tree["objects"]:
            for i in range(len(self.configer.get("details", "limb_seq"))):
                two_points = np.array(object["kpts"])[
                ...
                polygon = cv2.ellipse2Poly(
                    ...
                    (
                        int(length / 2),
                        self.configer.get("res", "vis_stick_width"),
                    ),
                    int(angle),
                    0,
                    360,
                ...
                image_canvas = cv2.addWeighted(
                    image_canvas, 0.4, cur_canvas, 0.6, 0
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        dest="image_file",
        help="The image file of Pose Parser.",
    )
    parser.add_argument(
        "--json_file",
        default=None,
        type=str,
        dest="json_file",
        help="The json file of Pose Parser.",
    )
    parser.add_argument(
        "--mask_file",
        default=None,
        type=str,
        dest="mask_file",
        help="The mask file of Pose Parser.",
    )
    parser.add_argument(
        "--image_dir",
        default=None,
        type=str,
        dest="image_dir",
        help="The image directory of Pose Parser.",
    )
    parser.add_argument(
        "--json_dir",
        default=None,
        type=str,
        dest="json_dir",
        help="The json directory of Pose Parser.",
    )
    parser.add_argument(
        "--mask_dir",
        default=None,
        type=str,
        dest="mask_dir",
        help="The mask directory of Pose Parser.",
    )

    args_parser = parser.parse_args()

    pose_parser = PoseParser(Configer(hypes_file=args_parser.hypes_file))
    pose_parser.parse_img_pose(
        args_parser.image_file, args_parser.json_file, args_parser.mask_file
    )
    pose_parser.parse_dir_pose(
        args_parser.image_dir, args_parser.json_dir, args_parser.mask_dir
    )
