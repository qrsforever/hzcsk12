...
class InsParser(object):
    ...
    def draw_bboxes(self, image_canvas, info_tree, conf_threshold=None):
        for object in info_tree["objects"]:
            class_name = self.configer.get("details", "name_seq")[
                object["label"]
            ]
            if "score" in object:
                if (
                    conf_threshold is not None
                    and float(object["score"]) < conf_threshold
                ):
                    continue
            ...
            cv2.rectangle(
                ...
                color=self.configer.get("details", "color_list")[
                    object["label"] % color_num
                ],
                thickness=3,
            )
            ...
            mask_color = self.configer.get("details", "color_list")[
                object["label"] % color_num
            ]
            mask_color = np.array(mask_color, dtype=np.uint8)
            mask_canvas = (
                np.repeat(maskmap[:, :, np.newaxis], 3, 2).astype(np.uint8)
                * mask_color
            )
            image_canvas = cv2.addWeighted(
                image_canvas, 0.6, mask_canvas, 0.5, 0
            )
