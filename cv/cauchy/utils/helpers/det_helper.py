...
from cauchy.extensions.ops.nms.nms_wrapper import nms
from cauchy.extensions.ops.nms.nms_wrapper import soft_nms


class DetHelper(object):
    @staticmethod
    def cls_nms(
        dets, labels, max_threshold=0.0, cls_keep_num=None, device_id=None
    ):
        ...
        for c in unique_labels:
            ...
            _, cls_keep = nms(
                dets[cls_index], iou_thr=max_threshold, device_id=device_id
            )
    ...
    def bbox_iou(box1, box2):
        ...
        lt = torch.max(
            box1[:, :2]
            .unsqueeze(1)
            .expand(N, M, 2),  # [N,2] -> [N,1,2] -> [N,M,2]
            box2[:, :2]
            .unsqueeze(0)
            .expand(N, M, 2),  # [M,2] -> [1,M,2] -> [N,M,2]
        )
    ...
    def bbox_kmeans(bboxes, cluster_number, dist=np.mean):
        ...
        np.random.seed()
        clusters = bboxes[
            np.random.choice(box_number, cluster_number, replace=False)
        ]  # init k clusters

        while True:
            distances = 1 - DetHelper.bbox_iou(
                torch.from_numpy(bboxes), torch.from_numpy(clusters)
            )
    ...
    def avg_iou(boxes, clusters):
        iou_matrix = DetHelper.bbox_iou(
            torch.from_numpy(boxes), torch.from_numpy(clusters)
        ).numpy()
