...
class MaskHelper(object):
    ...
    def polys2mask_wrt_box(polygons, box, target_size):
        ...
        rle = mask_util.frPyObjects(
            polygons_norm, target_size[1], target_size[0]
        )
        mask = np.array(mask_util.decode(rle), dtype=np.float32)
        # Flatten in case polygons was a list
        mask = np.sum(mask, axis=2)
    ...
    def rle_mask_voting(
        ...
        decoded_all_masks = [
            np.array(mask_util.decode(rle), dtype=np.float32)
            for rle in all_masks
        ]
        decoded_top_masks = [
            np.array(mask_util.decode(rle), dtype=np.float32)
            for rle in top_masks
        ]
        ...
        for k in range(len(top_masks)):
            ...
            rle = mask_util.encode(np.array(mask[:, :, np.newaxis], order="F"))[
                0
            ]
