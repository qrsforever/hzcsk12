...
from cauchy.utils.tools.progressbar import track_progress
...
class VideoReader(object):
    ...
    def __init__(self, filename, cache_capacity=10):
        FileHelper.check_file_exist(
            filename, "Video file not found: " + filename
        )
    ...
    def get_frame(self, frame_id):
        ...
        if frame_id < 0 or frame_id >= self._frame_cnt:
            raise IndexError(
                '"frame_id" must be between 0 and {}'.format(
                    self._frame_cnt - 1
                )
            )
    ...
    def cvt2frames(
        ...
        if show_progress:
            track_progress(
                write_frame, range(file_start, file_start + task_num)
            )
            ...
            for i in range(task_num):
                ...
                if img is None:
                    break
                filename = osp.join(
                    frame_dir, filename_tmpl.format(i + file_start)
                )
                cv2.imwrite(filename, img)
...
class VideoHelper(object):
    @staticmethod
    def convert_video(
        in_file, out_file, print_cmd=False, pre_options="", **kwargs
    ):
        ...
        cmd = "ffmpeg -y {} -i {} {} {}".format(
            pre_options, in_file, " ".join(options), out_file
        )
        if print_cmd:
            print(cmd)
    ...
    def frames2video(
        ...
        vwriter = cv2.VideoWriter(
            video_file, cv2.VideoWriter_fourcc(*fourcc), fps, resolution
        )
