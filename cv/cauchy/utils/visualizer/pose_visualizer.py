...
class PoseVisualizer(object):
    ...
    def vis_peaks(
        self, heatmap_in, ori_img_in, name="default", sub_dir="peaks"
    ):
        base_dir = os.path.join(
            self.configer.get("project_dir"), POSE_DIR, sub_dir
        )
        ...
        for j in range(self.configer.get("data", "num_kpts")):
            ...
            for peak in peaks:
                ori_img = cv2.circle(
                    ...
                    self.configer.get("details", "color_list")[j],
                    thickness=-1,
                )

            cv2.imwrite(
                os.path.join(base_dir, "{}_{}.jpg".format(name, j)), ori_img
            )
