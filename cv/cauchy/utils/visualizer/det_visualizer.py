...
from cauchy.datasets.tools.transforms import DeNormalize
...
class DetVisualizer(object):
    ...
    def vis_bboxes(self, image_in, bboxes_list, name="default", sub_dir="bbox"):
        """
          Show the diff bbox of individuals.
        """
        base_dir = os.path.join(
            self.configer.get("project_dir"), DET_DIR, sub_dir
        )
        ...
        image = cv2.resize(
            image, tuple(self.configer.get("data", "input_size"))
        )
    ...
    def vis_rois(
        ...
        for i in range(inputs.size(0)):
            ...
            img_path = os.path.join(
                base_dir, "{}_{}_{}.jpg".format(name, i, time.time())
            )

            cv2.imwrite(img_path, ori_img)

    def vis_default_bboxes(
        self,
        ori_img_in,
        default_bboxes,
        labels,
