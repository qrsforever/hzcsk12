...
class SegVisualizer(object):
    ...
    def vis_fn(
        self, preds, targets, ori_img_in=None, name="default", sub_dir="fn"
    ):
        base_dir = os.path.join(
            self.configer.get("project_dir"), SEG_DIR, sub_dir
        )
        ...
        for img_id in range(preds.shape[0]):
            ...
            pred = preds[img_id]
            result = np.zeros(
                shape=(pred.shape[0], pred.shape[1], 3), dtype=np.uint8
            )
            ...
            if ori_img_in is not None:
                image_result = cv2.addWeighted(
                    ori_img[i], 0.6, image_result, 0.4, 0
                )
    ...
    def vis_fp(
        self, preds, targets, ori_img_in=None, name="default", sub_dir="fp"
    ):
        base_dir = os.path.join(
            self.configer.get("project_dir"), SEG_DIR, sub_dir
        )
