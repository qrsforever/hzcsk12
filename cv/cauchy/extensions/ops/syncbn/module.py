...
from cauchy.extensions.syncbn.allreduce import allreduce
from cauchy.extensions.syncbn.comm import SyncMaster
...
syncbn = load(
    name="syncbn",
    sources=[
        "extensions/syncbn/src/operator.cpp",
...
class _batchnormtrain(Function):
    ...
    def backward(ctx, gradOutput):
        ...
        if gradOutput.is_cuda:
            gradInput, gradMean, gradStd, gradGamma, gradBeta = syncbn.batchnorm_backward(
                gradOutput, input, mean, std, gamma, beta, True
            )
        else:
            raise NotImplemented
...
class _SyncBatchNorm(_BatchNorm):
    ...
    def forward(self, input):
        ...
        # forward
        return batchnormtrain(
            input, mean, 1.0 / inv_std, self.weight, self.bias
        ).view(input_shape)
    ...
    def pull(self, igpu):
        ...
        with self.mutex:
            if igpu == 0:
                ...
                # flatten the tensors
                self.list = [
                    t for i in range(len(self.dict)) for t in self.dict[i]
                ]
