...
class PafGenerator(object):
    ...
    def __call__(self, gt_kpts, input_size, maskmap=None):
        ...
        accumulate_vec_map = np.zeros(
            (len(vec_pair) * 2, height, width), dtype=np.float32
        )
        ...
        for j in range(len(gt_kpts)):
            for i in range(channel):
                ...
                vec_map[:, yy, xx] = np.repeat(
                    mask[np.newaxis, :, :], 2, axis=0
                )
                vec_map[:, yy, xx] *= np.array([bax, bay])[
                    :, np.newaxis, np.newaxis
                ]

                mask = np.logical_or(
                    np.abs(vec_map[0:1, :, :]) > 0,
                    np.abs(vec_map[1:2, :, :]) > 0,
                )

                accumulate_vec_map[2 * i : 2 * i + 2, :, :] = np.multiply(
                    accumulate_vec_map[2 * i : 2 * i + 2], cnt[i : i + 1, :, :]
                )
                accumulate_vec_map[2 * i : 2 * i + 2, :, :] += vec_map
                cnt[i : i + 1, :, :][mask == 1] += 1
                mask = cnt[i : i + 1, :, :] == 0
                cnt[i : i + 1, :, :][mask == 1] = 1
                accumulate_vec_map[2 * i : 2 * i + 2, :, :] = np.divide(
                    accumulate_vec_map[2 * i : 2 * i + 2, :, :],
                    cnt[i : i + 1, :, :],
                )
                cnt[i : i + 1, :, :][mask == 1] = 0
