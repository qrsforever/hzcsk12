...
from cauchy.datasets.pose.utils.paf_generator import PafGenerator
...
class OpenPoseLoader(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.aug_transform is not None and len(bboxes) > 0:
            img, maskmap, kpts, bboxes = self.aug_transform(
                img, maskmap=maskmap, kpts=kpts, bboxes=bboxes
            )

        elif self.aug_transform is not None:
            img, maskmap, kpts = self.aug_transform(
                img, maskmap=maskmap, kpts=kpts
            )
