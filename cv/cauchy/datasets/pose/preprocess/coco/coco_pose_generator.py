...
class CocoPoseGenerator(object):
    def __init__(
        self, args, json_dir=JOSN_DIR, mask_dir=MASK_DIR, image_dir=IMAGE_DIR
    ):
        self.args = args
    ...
    def generate_label(self):
        for i, img_id in enumerate(self.img_ids):
            ...
            for p in range(num_persons):
                ...
                if (
                    img_anns[p]["num_keypoints"] < 5
                    or img_anns[p]["area"] < 32 * 32
                ):
                    mask_list.append(p)
                    continue
                ...
                for pc in person_centers:
                    dis = math.sqrt(
                        (person_center[0] - pc[0]) * (person_center[0] - pc[0])
            ...
            if len(persons) > 0:
                ...
                fw = open(
                    os.path.join(
                        self.json_dir, "{}.json".format(filename.split(".")[0])
                    ),
                    "w",
                )
                ...
                mask_image.save(
                    os.path.join(
                        self.mask_dir, "{}.png".format(filename.split(".")[0])
                    )
                )
                ...
                mask_image_vis.save(
                    os.path.join(
                        self.mask_dir,
                        "{}_vis.png".format(filename.split(".")[0]),
                    )
                )
                shutil.copyfile(
                    os.path.join(self.args.img_dir, filename),
    ...
    def __coco_to_ours(self, persons):
        ...
        for person in persons:
            ...
            dic["kpts"][1][0] = (
                person["kpts"][5][0] + person["kpts"][6][0]
            ) * 0.5
            dic["kpts"][1][1] = (
                person["kpts"][5][1] + person["kpts"][6][1]
            ) * 0.5
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        dest="root_dir",
        help="The directory to save the ground truth.",
    )
    parser.add_argument(
        "--ori_anno_file",
        default=None,
        type=str,
        dest="anno_file",
        help="The annotations file of coco keypoints.",
    )
    parser.add_argument(
        "--ori_img_dir",
        default=None,
        type=str,
        dest="img_dir",
        help="The image dir corresponding to coco anno file.",
    )
