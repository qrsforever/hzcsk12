    ...
    def __call__(
        ...
        for _ in range(50):
            scale = random.uniform(
                self.up_scale_range[0], self.up_scale_range[1]
            )
    ...
    def __call__(
        ...
        if kpts is not None and kpts.size > 0:
            ...
            mask = np.logical_or.reduce(
                (
                    kpts[:, :, 0] >= target_size[0],
                    kpts[:, :, 0] < 0,
                    kpts[:, :, 1] >= target_size[1],
                    kpts[:, :, 1] < 0,
                )
            )
            kpts[mask == 1, 2] = -1
        ...
        if bboxes is not None and bboxes.size > 0:
            if self.allow_outside_center:
                ...
                crop_bb = np.array(
                    [
                        offset_left,
                        offset_up,
                        offset_left + target_size[0],
                        offset_up + target_size[1],
                    ]
                )
                center = (bboxes[:, :2] + bboxes[:, 2:]) / 2
                mask = np.logical_and(
                    crop_bb[:2] <= center, center < crop_bb[2:]
                ).all(axis=1)
            ...
            mask = np.logical_and(
                mask, (bboxes[:, :2] < bboxes[:, 2:]).all(axis=1)
            )
            bboxes = bboxes[mask]
                ...
                for object_id in range(len(polygons)):
                    if mask[object_id] == 1:
                        for polygon_id in range(len(polygons[object_id])):
                            ...
                            polygons[object_id][polygon_id][0::2] = np.clip(
                                polygons[object_id][polygon_id][0::2],
                                0,
                                target_size[0] - 1,
                            )
                            polygons[object_id][polygon_id][1::2] = np.clip(
                                polygons[object_id][polygon_id][1::2],
                                0,
                                target_size[1] - 1,
                            )
        ...
        expand_image[
            ...
            abs(min(offset_left, 0)) : abs(min(offset_left, 0)) + width,
        ] = img
        img = expand_image[
        ...
        if labelmap is not None:
            expand_labelmap = np.zeros(
                (
                    max(height, target_size[1]) + abs(offset_up),
                    max(width, target_size[0]) + abs(offset_left),
                ),
                dtype=labelmap.dtype,
            )
            expand_labelmap[:, :] = 255
            expand_labelmap[
                abs(min(offset_up, 0)) : abs(min(offset_up, 0)) + height,
    ...
    def __call__(
        ...
        if polygons is not None:
            for object_id in range(len(polygons)):
                for polygon_id in range(len(polygons[object_id])):
                    polygons[object_id][polygon_id][0::2] = (
                        width - 1 - polygons[object_id][polygon_id][0::2]
                    )
...
class RandomResize(object):
    ...
    def __init__(
        ...
        if target_size is not None:
            if isinstance(target_size, int):
                ...
                isinstance(target_size, (list, tuple)) and len(target_size) == 2
            ):
    ...
    def get_scale(self, img_size, bboxes):
        if self.method == "random":
            ...
            if (
                ...
                scale = 0.6 / max(
                    max(border[:, 0]) / self.input_size[0],
                    max(border[:, 1]) / self.input_size[1],
                )
                scale_ratio = (
    ...
    def __call__(
        ...
        converted_size = (
            int(width * w_scale_ratio),
            int(height * h_scale_ratio),
        )

        img = cv2.resize(
            img, converted_size, interpolation=cv2.INTER_CUBIC
        ).astype(np.uint8)
        if labelmap is not None:
            labelmap = cv2.resize(
                labelmap, converted_size, interpolation=cv2.INTER_NEAREST
            )

        if maskmap is not None:
            maskmap = cv2.resize(
                maskmap, converted_size, interpolation=cv2.INTER_NEAREST
            )
...
class RandomRotate(object):
    ...
    def __call__(
        ...
        rotate_mat[0, 2] += (new_width / 2.0) - img_center[0]
        rotate_mat[1, 2] += (new_height / 2.0) - img_center[1]
        img = cv2.warpAffine(
            img, rotate_mat, (new_width, new_height), borderValue=self.mean
        ).astype(np.uint8)
        if labelmap is not None:
            labelmap = cv2.warpAffine(
                labelmap,
                rotate_mat,
        ...
        if bboxes is not None and bboxes.size > 0:
            for i in range(len(bboxes)):
                ...
                bboxes[i] = [
                    min(bbox_temp[0], bbox_temp[2], bbox_temp[4], bbox_temp[6]),
...
class RandomCrop(object):
    ...
    def __init__(
        ...
        if isinstance(crop_size, float):
            ...
            isinstance(crop_size, collections.Iterable) and len(crop_size) == 2
        ):
            self.size = crop_size
        else:
    ...
    def get_lefttop(self, crop_size, img_size):
        if self.method == "center":
            return [
                (img_size[0] - crop_size[0]) // 2,
                (img_size[1] - crop_size[1]) // 2,
            ]
    ...
    def __call__(
        ...
        img = img[
            offset_up : offset_up + target_size[1],
            offset_left : offset_left + target_size[0],
        ]
        if maskmap is not None:
            maskmap = maskmap[
                offset_up : offset_up + target_size[1],
...
class RandomFocusCrop(object):
    ...
    def get_center(self, img_size, bboxes):
        if bboxes is None or bboxes.size == 0:
            if img_size[0] > self.size[0]:
                x = random.randint(
                    self.size[0] // 2, img_size[0] - self.size[0] // 2
                )
            else:
                x = img_size[0] // 2
            ...
            if img_size[1] > self.size[1]:
                y = random.randint(
                    self.size[1] // 2, img_size[1] - self.size[1] // 2
                )
            else:
                y = img_size[1] // 2
            ...
            max_center = [
                (bboxes[max_index][0] + bboxes[max_index][2]) / 2,
    ...
    def __call__(
        ...
        expand_image = np.zeros(
            (
                max(height, self.size[1]) + abs(offset_up),
                max(width, self.size[0]) + abs(offset_left),
                channels,
            ),
            dtype=img.dtype,
        )
        ...
        img = expand_image[
            max(offset_up, 0) : max(offset_up, 0) + self.size[1],
    ...
    def __call__(
        ...
        while True:
            ...
            for _ in range(50):
                ...
                max_ratio = min(2.0, 1.0 / scale / scale)
                ratio = math.sqrt(random.uniform(min_ratio, max_ratio))
...
class Resize(object):
    ...
    def __init__(
        self, target_size=None, min_side_length=None, max_side_length=None
    ):
    ...
    def __call__(
        ...
        if labelmap is not None:
            labelmap = cv2.resize(
                labelmap, target_size, interpolation=cv2.INTER_NEAREST
            )

        if maskmap is not None:
            maskmap = cv2.resize(
                maskmap, target_size, interpolation=cv2.INTER_NEAREST
            )
...
class CV2AugCompose(object):
    ...
    def __init__(self, configer, split="train"):
        ...
        for trans in aug_trans["trans_seq"] + shuffle_train_trans:
            self.transforms[trans] = CV2_AUGMENTATIONS_DICT[trans](
                **aug_trans[trans]
            )
    ...
    def __call__(
        ...
        if "shuffle_trans_seq" in aug_trans:
            if isinstance(aug_trans["shuffle_trans_seq"][0], list):
                ...
                shuffle_trans_seq = shuffle_trans_seq_list[
                    random.randint(0, len(shuffle_trans_seq_list))
                ]
            else:
