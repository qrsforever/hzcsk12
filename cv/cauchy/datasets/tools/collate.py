...
from cauchy.utils.helpers.tensor_helper import TensorHelper
...
def stack(batch, data_key=None, trans_dict=None):
    if isinstance(batch[0][data_key], DataContainer):
        if batch[0][data_key].stack:
            assert (
                isinstance(batch[0][data_key].data, torch.Tensor)
                or isinstance(batch[0], int_classes)
                or isinstance(batch[0], float)
                or isinstance(batch[0], string_classes)
                or isinstance(batch[0], collections.Mapping)
                or isinstance(batch[0], collections.Sequence)
            )
            stacked = []
                    ...
                    stacked.append(
                        default_collate(
                            [
                                ...
                                for sample in batch[
                                    i : i + trans_dict["samples_per_gpu"]
                                ]
                            ]
                        )
                    )
...
def collate(batch, trans_dict):
    ...
    if trans_dict["size_mode"] == "fix_size":
        ...
        target_width, target_height = ms_input_size[
            random.randint(0, len(ms_input_size) - 1)
        ]
        ...
        raise NotImplementedError(
            "Size Mode {} is invalid!".format(trans_dict["size_mode"])
        )
    ...
    if "fit_stride" in trans_dict:
        ...
        pad_w = (
            ...
            else stride - (target_width % stride)
        )  # right
        pad_h = (
            0
            if (target_height % stride == 0)
            else stride - (target_height % stride)
        )  # down
        # print("qrs pad_w = [%d] pad_h = [%d]" % (pad_w, pad_h))
    ...
    for i in range(len(batch)):
        ...
        if trans_dict["align_method"] in ["only_scale", "scale_and_pad"]:
            ...
            # print(scaled_size, width, w_scale_ratio, height, h_scale_ratio)
            ...
            if "maskmap" in data_keys:
                batch[i]["maskmap"] = DataContainer(
                    TensorHelper.resize(
                        batch[i]["maskmap"].data, scaled_size_hw, mode="nearest"
                    ),
                    stack=batch[i]["maskmap"].stack,
                )

        # print("qrs target_width[%d], scaled_size[0]=%d" % (target_width, scaled_size[0]))
        # print("qrs target_h[%d], scaled_size[1]=%d" % (target_height, scaled_size[1]))
        ...
        if pad_width > 0 or pad_height > 0:
            ...
            if (
                "pad_mode" not in trans_dict
                or trans_dict["pad_mode"] == "random"
            ):
            ...
            batch[i]["img"] = DataContainer(
                F.pad(batch[i]["img"].data, pad=pad, value=0),
                stack=batch[i]["img"].stack,
            )
