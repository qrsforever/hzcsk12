...
class RandomPad(object):
    ...
    def __call__(
        ...
        img = ImageOps.expand(
            img, (left_pad, up_pad, right_pad, down_pad), fill=self.mean
        )

        if labelmap is not None:
            labelmap = ImageOps.expand(
                labelmap, (left_pad, up_pad, right_pad, down_pad), fill=255
            )

        if maskmap is not None:
            maskmap = ImageOps.expand(
                maskmap, (left_pad, up_pad, right_pad, down_pad), fill=1
            )
...
class Padding(object):
    ...
    def __call__(
        ...
        img = ImageOps.expand(
            img, border=tuple(self.pad), fill=tuple(self.mean)
        )
        if maskmap is not None:
        ...
        if labelmap is not None:
            labelmap = ImageOps.expand(
                labelmap, border=tuple(self.pad), fill=255
            )
...
class RandomCrop(object):
    ...
    def __call__(
        ...
        target_size = (
            min(self.size[0], img.size[0]),
            min(self.size[1], img.size[1]),
        )
...
class PILAugCompose(object):
    ...
    def __init__(self, configer, split="train"):
        ...
        for trans in aug_trans["trans_seq"] + shuffle_train_trans:
            self.transforms[trans] = PIL_AUGMENTATIONS_DICT[trans](
                **aug_trans[trans]
            )
