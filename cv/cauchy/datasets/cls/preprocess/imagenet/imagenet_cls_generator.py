...
class ImageNetClsGenerator(object):
    def __init__(self, args, image_dir=IMAGE_DIR):
        self.args = args
        self.train_json_file = os.path.join(
            self.args.save_dir, "train/label.json"
        )
    ...
    def generate_label(self):
        ...
        for i in range(NUM_OF_CLASSES):
            train_folder = os.path.join(
                self.args.ori_img_dir, "train", imagenet_class_dict[str(i)][0]
            )
            val_folder = os.path.join(
                self.args.ori_img_dir, "val", imagenet_class_dict[str(i)][0]
            )
            ...
            for image_file in os.listdir(val_folder):
                ...
                shutil.copy(
                    os.path.join(val_folder, image_file),
