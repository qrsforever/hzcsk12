...
class ADE20KSegGenerator(object):
    ...
    def generate_label(self):
        train_img_folder = os.path.join(
            self.args.ori_root_dir, "images/training"
        )
        train_mask_folder = os.path.join(
            self.args.ori_root_dir, "annotations/training"
        )

        val_img_folder = os.path.join(
            self.args.ori_root_dir, "images/validation"
        )
        val_mask_folder = os.path.join(
            self.args.ori_root_dir, "annotations/validation"
        )
        ...
        for filename in os.listdir(train_img_folder):
            ...
            if filename.endswith(".jpg"):
                ...
                if os.path.isfile(maskpath):
                    shutil.copy(
                        imgpath, os.path.join(self.train_image_dir, filename)
                    )
                    shutil.copy(
                        maskpath, os.path.join(self.train_label_dir, maskname)
                    )
                else:
        ...
        for filename in os.listdir(val_img_folder):
            ...
            if filename.endswith(".jpg"):
                ...
                if os.path.isfile(maskpath):
                    shutil.copy(
                        imgpath, os.path.join(self.val_image_dir, filename)
                    )
                    shutil.copy(
                        maskpath, os.path.join(self.val_label_dir, maskname)
                    )
                else:
                    print("cannot find the mask:", maskpath)
