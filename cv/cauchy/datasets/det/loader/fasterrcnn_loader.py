...
class FasterRCNNLoader(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.aug_transform is not None:
            img, bboxes, labels = self.aug_transform(
        ...
        return dict(
            img=DataContainer(
                img, stack=True, return_dc=True, samples_per_gpu=True
            ),
            bboxes=DataContainer(
                bboxes, stack=False, return_dc=True, samples_per_gpu=True
            ),
            labels=DataContainer(
                labels, stack=False, return_dc=True, samples_per_gpu=True
            ),
            meta=DataContainer(
                meta,
                stack=False,
    ...
    def __read_json_file(self, json_file):
        ...
        for object in json_dict["objects"]:
            if (
                "difficult" in object
                and object["difficult"]
                and not self.configer.get("data", "keep_difficult")
            ):
                continue
