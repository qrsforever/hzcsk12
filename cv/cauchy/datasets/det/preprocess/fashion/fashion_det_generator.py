...
class FashionDetGenerator(object):
    ...
    def generate_label(self):
        ...
        for (type_line, bbox_line) in zip(type_lines, bbox_lines):
            ...
            object_dict = dict()
            object_dict["bbox"] = [
                float(bbox_line_items[1]),
                float(bbox_line_items[2]),
                float(bbox_line_items[3]),
                float(bbox_line_items[4]),
            ]

            object_dict["label"] = int(type_line_items[1]) - 1
            max_cat = (
                int(type_line_items[1])
                if int(type_line_items[1]) > max_cat
                else max_cat
            )
            ...
            if file_count % self.args.val_interval == 0:
                ...
                fw.close()
                shutil.copy(
                    os.path.join(self.args.ori_img_dir, file_name),
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        dest="ori_type_file",
        help="The directory of the type data.",
    )
    parser.add_argument(
        "--ori_bbox_file",
        default=None,
        type=str,
        dest="ori_bbox_file",
        help="The directory of the bbox data.",
    )
    parser.add_argument(
        "--val_interval",
        default=10,
        type=float,
        dest="val_interval",
        help="The ratio of train & val data.",
    )
