...
class VocDetGenerator(object):
    def __init__(self, args, json_dir=JOSN_DIR, image_dir=IMAGE_DIR):
        self.args = args
        self.train_json_dir = os.path.join(
            self.args.save_dir, "train", json_dir
        )
    ...
    def _get_info_tree(self, label_file, dataset="VOC2007"):
        label_file_path = os.path.join(
            self.args.root_dir, dataset, "Annotations", label_file
        )
        object_list = list()
        tree_dict = dict()
            ...
            for obj in xml_tree.findAll("object"):
                ...
                for name_tag in obj.findChildren("name"):
                    ...
                    if name in CAT_DICT:
                        ...
                        xmin = (
                            int(float(bbox.findChildren("xmin")[0].contents[0]))
                            - 1
                        )
                        ymin = (
                            int(float(bbox.findChildren("ymin")[0].contents[0]))
                            - 1
                        )
                        xmax = (
                            int(float(bbox.findChildren("xmax")[0].contents[0]))
                            - 1
                        )
                        ymax = (
                            int(float(bbox.findChildren("ymax")[0].contents[0]))
                            - 1
                        )
    ...
    def generate_label(self):
        ...
        if self.args.dataset in ["VOC07", "VOC07+12", "VOC07++12"]:
            with open(
                os.path.join(
                    self.args.root_dir, "VOC2007/ImageSets/Main/trainval.txt"
                ),
                "r",
            ) as train_stream:
                for img_name in train_stream.readlines():
                    ...
                    file_count += 1
                    tree_dict = self._get_info_tree(
                        label_file, dataset="VOC2007"
                    )
                    fw = open(
                        os.path.join(
                            self.train_json_dir, "{}.json".format(img_name)
                        ),
                        "w",
                    )
                    ...
                    shutil.copy(
                        ...
                        os.path.join(
                            self.train_image_dir, "{}.jpg".format(img_name)
                        ),
                    )
        ...
        if self.args.dataset in ["VOC07+12", "VOC07++12", "VOC12"]:
            with open(
                os.path.join(
                    self.args.root_dir, "VOC2012/ImageSets/Main/trainval.txt"
                ),
                "r",
            ) as train_stream:
                for img_name in train_stream.readlines():
        ...
        if self.args.dataset in ["VOC07++12"]:
            with open(
                os.path.join(
                    self.args.root_dir, "VOC2007/ImageSets/Main/test.txt"
                ),
                "r",
            ) as train_stream:
                for img_name in train_stream.readlines():
        ...
        with open(
            os.path.join(self.args.root_dir, "VOC2007/ImageSets/Main/test.txt"),
            "r",
        ) as train_stream:
            for img_name in train_stream.readlines():
                img_name = img_name.rstrip()
                ...
                tree_dict = self._get_info_tree(label_file)
                fw = open(
                    os.path.join(self.val_json_dir, "{}.json".format(img_name)),
                    "w",
                )
                fw.write(json.dumps(tree_dict))
                fw.close()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        dest="root_dir",
        help="The directory of the voc root.",
    )
    parser.add_argument(
        "--dataset",
        default=None,
        type=str,
        dest="dataset",
        help="The target dataset that will be generated.",
    )
